...
export const NetworkBreakdown: React.FC = () => {
    ...
    const data = React.useMemo(() => {
        ...
        organicPosts.forEach(post => {
            ...
            networkStats[post.network].shares += post.shares;
        });
        ...
        return Object.entries(networkStats).map(([name, stats]) => {
            ...
            else if (metric === 'engagementRate') value = stats.impressions > 0 ? (stats.engagements * 100) / stats.impressions : 0;
            else if (metric === 'shareRatio') value = stats.engagements > 0 ? (stats.shares * 100) / stats.engagements : 0;
            ...
            <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Select value={metric} onValueChange={(val: Metric) => setMetric(val)}>
                    ...
                    <SelectContent>
                        ...
                        <SelectItem value="shareRatio" className="text-[10px] font-bold uppercase">Share Ratio %</SelectItem>
                    </SelectContent>
                </Select>
            </CardHeader>
